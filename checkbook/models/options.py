...
class Options(BaseModel):
    ...
    Options
    """ # noqa: E501
    return_code: Optional[StrictStr] = Field(default=None, description="Desired return code")
    __properties: ClassVar[List[str]] = ["return_code"]

    @field_validator('return_code')
    def return_code_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['R01', 'R02', 'R03', 'A', 'B', 'C', 'D', '04', '05', '06', 'R901']):
            raise ValueError("must be one of enum values ('R01', 'R02', 'R03', 'A', 'B', 'C', 'D', '04', '05', '06', 'R901')")
        return value
